// Package httpserver provides unified HTTP server construction on top of
// net/http.
//
// New builds the server from explicitly set options, Start serves it in a
// background goroutine and Stop shuts it down: gracefully first, forcefully
// once the shutdown timeout expires. Notify reports the serve outcome: a nil
// value means the server stopped as requested, a non-nil one means a failure.
...
// Server provides access to the HTTP server.
type Server interface {
	...
	// Unwrap returns the underlying *http.Server.
	Unwrap() *http.Server
}
...
type server struct {
	...
	httpConfigure   func(*http.Server)
...
// New builds an HTTP server from opts. The listen address and the handler are
// required and every option is validated before the underlying server is
// created.
func New(opts ...Option) (*server, error) {
	...
	if s.httpConfigure != nil {
		s.httpConfigure(s.innerServer)
	}
	// Assigned after the configure pass, so the handler from WithHandler stays
	// authoritative.
	s.innerServer.Handler = s.handler
...
func (s *server) Start(ctx context.Context) error {
	...
	if !isNotStarted {
		return fmt.Errorf("httpserver: server is already started")
	}
...
func (s *server) Stop() {
	...
	if err := s.innerServer.Shutdown(ctx); err != nil {
		// Graceful shutdown did not finish in time: drop the remaining
		// connections.
		_ = s.innerServer.Close()
	}
}
...
func (s *server) serve(ctx context.Context) {
	...
	if err != nil {
		err = fmt.Errorf("httpserver: failed to listen on %s: %w", s.address, err)
		s.logError(ctx, "httpserver: server start failed", err)
		s.notify <- err
		return
	}
	...
	if s.stopRequested.Load() && errors.Is(serveErr, http.ErrServerClosed) {
	...
	if serveErr != nil {
		s.logError(ctx, "httpserver: serve failed", serveErr)
	}
...
func (s *server) logStarted(ctx context.Context) {
	...
	s.logger.Info(ctx, "httpserver: server started", slog.String("address", s.address))
}
